...
public class CameraRespawnState : CameraStateBase
{
    ...
    public override void EnterState(CameraManager camera)
    {
        ...
        CameraManagerRef.VirtualCameraCombat.GetCinemachineComponent<Cinemachine3rdPersonFollow>().CameraDistance = CameraManagerRef.CameraDistanceRespawn;
    ...
    {
        if (CameraManagerRef.VirtualCameraCombat.GetCinemachineComponent<Cinemachine3rdPersonFollow>().CameraDistance >= 7)
            CameraManagerRef.VirtualCameraCombat.GetCinemachineComponent<Cinemachine3rdPersonFollow>().CameraDistance -= Time.deltaTime * CameraManagerRef.MultiplyTimeForDistanceWhenRespawn /* CameraManagerRef.ValueRemoveForDistanceWhenRespawn*/;

        if (CameraManagerRef.VirtualCameraCombat.GetCinemachineComponent<Cinemachine3rdPersonFollow>().CameraDistance <= 7 && CameraManagerRef.CameraAngleOverride <= 0)
        {
